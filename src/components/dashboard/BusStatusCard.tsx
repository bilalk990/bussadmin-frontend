      ...
      <div className="space-y-2 sm:space-y-4">
        ...
        {bus.status === 'maintenance' && (
          <div className="text-[10px] sm:text-sm text-gray-400">
            ...
            <p className="mt-0.5 sm:mt-1">Last updated: {bus.updatedAt ? new Date(bus.updatedAt).toLocaleDateString() : 'N/A'}</p>
          </div>
        )}
        ...
        {bus.status === 'inactive' && (
          <div className="text-[10px] sm:text-sm text-gray-400">
            ...
            <p className="mt-0.5 sm:mt-1">Last active: {bus.lastActive ? new Date(bus.lastActive).toLocaleDateString() : (bus.updatedAt ? new Date(bus.updatedAt).toLocaleDateString() : 'N/A')}</p>
          </div>
        )}
      </div>
    </motion.div>
