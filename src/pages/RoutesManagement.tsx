...
const RoutesManagement = () => {
  ...
  const avgPassengers = Math.round(
    routes.reduce((sum, route) => sum + (route.adultPrice || 0), 0) / (routes.length || 1)
  );
                ...
                filteredRoutes.map((route) => (
                  <div key={route._id} className="bg-gray-900/50 rounded-lg p-3 space-y-2">
                    <div className="flex items-start justify-between">
                      ...
                      <span className={`inline-flex px-2 py-1 rounded-full text-xs font-medium ${route.status === 'active' ? 'bg-success-900/60 text-success-400' :
