...
const ScheduleManagement = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const [schedulesResult, routesResult, driversResult] = await Promise.allSettled([
  ...
  useEffect(() => {
    const fetchFormData = async () => {
      ...
      try {
        ...
        const [routesResult, driversResult] = await Promise.allSettled([
          tripHistoryService.getAllRoutes(),
          tripHistoryService.getAvailableDrivers()
        ]);
  ...
  const handleCreateSchedule = async () => {
    ...
    try {
      ...
      // Check selected driver has a bus
      const selectedDriver = drivers.find(d => d._id === formData.driverId);
      if (selectedDriver && !selectedDriver.bus) {
        setFormError('Selected driver has no bus assigned. Please assign a bus to this driver first.');
        return;
      }
      ...
      if (!formData.arrivalTime) {
        setFormError('Please select an arrival time');
        return;
      }
      if (!formData.departureBusStation) {
        setFormError('Please enter departure bus station');
        return;
      }
      if (!formData.arrivalBusStation) {
        setFormError('Please enter arrival bus station');
        return;
      }
  ...
  return (
    ...
    >
      {/* Error Banner */}
      {error && (
        <div className="flex items-center gap-3 p-3 bg-warning-900/40 border border-warning-700 rounded-lg text-warning-400 text-sm">
          <AlertCircle size={16} className="shrink-0" />
          <span>{error}</span>
          <button onClick={() => setError(null)} className="ml-auto text-warning-400 hover:text-white">
            <X size={14} />
          </button>
        </div>
      )}
              ...
              {isFormLoading ? (
                ...
                <div className="space-y-2 sm:space-y-4 overflow-y-auto flex-1 pr-1 sm:pr-2">
                  ...
                  <div>
                    ...
                    {drivers.length === 0 ? (
                      <div className="p-2 sm:p-3 bg-gray-800 border border-gray-700 rounded-md text-xs sm:text-sm text-gray-400">
                        No drivers found. Please add a driver first from the Drivers page.
                      </div>
                    ) : (
                      <select
                      ...
                      >
                        ...
                        {drivers.map((driver) => (
                          <option key={driver._id} value={driver._id}>
                            {driver.name} ({driver.email}){driver.bus ? ` — Bus: ${driver.bus.name} [${driver.bus.plateNumber}]` : ' — ⚠ No bus assigned'}
                          </option>
                        ))}
                      </select>
                    )}
                    {/* Show selected driver's bus info */}
                    {formData.driverId && (() => {
                      const selectedDriver = drivers.find(d => d._id === formData.driverId);
                      if (!selectedDriver) return null;
                      if (!selectedDriver.bus) {
                        return (
                          <p className="mt-1 text-xs text-red-400">
                            ⚠ This driver has no bus assigned. Go to Fleet Management → assign a bus to this driver first.
                          </p>
                        );
                      }
                      return (
                        <p className="mt-1 text-xs text-green-400">
                          Bus: {selectedDriver.bus.name} [{selectedDriver.bus.plateNumber}] — Status: {selectedDriver.bus.status}
                        </p>
                      );
                    })()}
                  </div>
                ...
                >
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 sm:gap-4">
                    ...
                    <div className="flex items-center gap-2 sm:gap-4">
                      ...
                      <ChevronDown
                        size={16}
                        className={`text-gray-400 transition-transform duration-200 ${selectedSchedule === schedule._id ? 'rotate-180' : ''
                          }`}
                      />
                    </div>
                  </div>
