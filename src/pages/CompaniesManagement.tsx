...
const CompaniesManagement: React.FC = () => {
  ...
  const handleEditCompany = async (formData: any) => {
    ...
    try {
      const updatedCompany = await companyService.updateCompany(selectedCompany.id, formData);
  ...
  const handleSuspendCompany = async (companyId: string) => {
    try {
      ...
      setCompanies(companies.map(company =>
        company.id === companyId ? { ...company, isActive: false } : company
      ));
    } catch (error) {
  ...
  const handleActivateCompany = async (companyId: string) => {
    try {
      ...
      setCompanies(companies.map(company =>
        company.id === companyId ? { ...company, isActive: true } : company
      ));
    } catch (error) {
  ...
  const filteredCompanies = companies.filter(company => {
    ...
    const matchesSearch = (company.agencyName?.toLowerCase() || '').includes(searchTerm.toLowerCase()) ||
            ...
            >
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-3">
                  <img
                    src={company.logo}
                    alt={company.agencyName}
                    className="w-12 h-12 rounded-full object-cover"
                  />
                  <div>
                    <h3 className="text-lg font-semibold text-white">{company.agencyName}</h3>
                    <p className="text-sm text-gray-400">{company.contactEmail}</p>
                  </div>
                </div>
                <div className="relative">
                  <button
                    onClick={() => setSelectedMenuCompany(selectedMenuCompany === company.id ? null : company.id)}
                    className="p-1 hover:bg-gray-700/50 rounded-lg transition-colors"
                  >
                    <MoreVertical size={20} className="text-gray-400" />
                  </button>

                  {selectedMenuCompany === company.id && (
                    <div className="absolute right-0 mt-2 w-48 bg-gray-800 rounded-lg shadow-lg py-1 z-10">
                      <button
                        ...
                        disabled={processingViewDetailsId === company.id}
                      >
                        {processingViewDetailsId === company.id ? (
                      ...
                      {company.isActive ? (
                        <button
                          ...
                          disabled={processingSuspendId === company.id}
                        >
                          {processingSuspendId === company.id ? (
                            <Loader2 size={16} className="animate-spin" />
                          ) : (
                            <XCircle size={16} />
                          )}
                        ...
                        <button
                          ...
                          disabled={processingActivateId === company.id}
                        >
                          {processingActivateId === company.id ? (
                            <Loader2 size={16} className="animate-spin" />
                          ) : (
                      ...
                      <button
                        ...
                        className="flex items-center gap-2 w-full px-4 py-2 text-sm text-red-400 hover:bg-gray-700/50"
                        disabled={processingDeleteId === company.id}
                      >
                        {processingDeleteId === company.id ? (
                          <Loader2 size={16} className="animate-spin" />
                        ) : (
                          <Trash2 size={16} />
                        )}
              ...
              <div className="flex items-center justify-between pt-2 border-t border-gray-700/50">
                <div className="flex items-center gap-2">
                  <span className={`px-2 py-1 rounded-full text-xs ${company.isActive ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400'
                    }`}>
            ...
            <div className="space-y-6">
              <div className="flex items-center gap-4">
                <img
                  ...
                  alt={selectedCompany.agencyName}
                  className="w-20 h-20 rounded-full object-cover"
                />
                <div>
                  <h3 className="text-xl font-semibold text-white">{selectedCompany.agencyName}</h3>
